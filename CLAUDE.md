...
Sanderling reads the UI tree out of the memory of a running 64-bit EVE Online client process (read-only — it never injects into or writes to the game client). Three components live here:
...
Sometimes it is not alpha either: the client leaves the node opaque, full size, fully labelled, and simply stops hit-testing it. `_pickState` is the only evidence — 0 takes no mouse input and neither does anything below it, 1 takes it, 2 passes it to the children. The AIR career program stacks its careers / activities / goals rings at one rect and hides two of them this way. `_pickState` 0 does *not* mean "hidden" on its own: it is 0 on plain labels inside live controls, and on a control the client greys out. A control that is 0 **and** states no `enabled` / `isDisabled` / `_enabled` / `_interaction_state` flag is one the client is not showing — that is `isSilentlyInert` in `GenericWindow.elm`.
...
3. The client derives the typed character from `WM_KEYDOWN` itself, so a key-down and a `WM_CHAR` for the same character enter it **twice**. They are alternatives, not a pair. Type text with `WM_CHAR` alone (`TypeCharacter` in the effect vocabulary) and reserve the key effects for keys that act — Backspace, Return, the arrows. Key-downs cannot type text properly: a posted modifier does not register as held (a posted Ctrl+A typed a literal `a`), so every capital and shifted symbol is out of reach and text arrives lowercase and unpunctuated. `WM_CHAR` carries the character itself and has neither limit — measured 2026-07-26, `XY, it's!` arrived exactly.
