...
class Program
{
    ...
    static int Main(string[] args)
    {
        var app =
            new CommandLineApplication
            {
                ...
                Description =
                "Welcome to the Sanderling memory reading command-line interface. This tool helps you read objects from the memory of a 64-bit EVE Online client process and save it to a file. In addition to that, you have the option to save the entire memory contents of a game client process to a file.\nTo get help or report an issue, see the project website at https://github.com/Arcitectus/Sanderling",
            };
        ...
        app.Command(
            ...
            {
                saveProcessSampleCmd.Description =
                    "Save a sample from a live process to a file. Use the '--pid' parameter to specify the process id.";
                ...
                var delaySecondsParam =
                    saveProcessSampleCmd.Option(
                        "--delay",
                        "Timespan to wait before starting the collection of the sample, in seconds.",
                        CommandOptionType.SingleValue);
        ...
        app.Command(
            ...
            {
                readMemoryEveOnlineCmd.Description =
                    "Read the memory of an 64 bit EVE Online client process. You can use a live process ('--pid') or a process sample file ('--source-file') as the source.";

                var processIdParam =
                    readMemoryEveOnlineCmd.Option(
                        "--pid",
                        "Id of the Windows process to read from.",
                        CommandOptionType.SingleValue);

                var rootAddressParam =
                    readMemoryEveOnlineCmd.Option(
                        "--root-address",
                        "Address of the UI root. If the address is not specified, the program searches the whole process memory for UI roots.",
                        CommandOptionType.SingleValue);

                var sourceFileParam =
                    readMemoryEveOnlineCmd.Option(
                        "--source-file",
                        "Process sample file to read from.",
                        CommandOptionType.SingleValue);
                ...
                var removeOtherDictEntriesParam =
                    readMemoryEveOnlineCmd.Option(
                        ...
                        "Use this to remove the other dict entries from the UI nodes in the resulting JSON representation.",
                        CommandOptionType.NoValue);

                var warmupIterationsParam =
                    readMemoryEveOnlineCmd.Option(
                        "--warmup-iterations",
                        "Only to measure execution time: Use this to perform additional warmup runs before measuring execution time.",
                        CommandOptionType.SingleValue);
                ...
                readMemoryEveOnlineCmd.OnExecute(
                    () =>
                    {
                        ...
                        (IMemoryReader, IImmutableList<ulong>) GetMemoryReaderAndRootAddressesFromProcessSampleFile(byte[] processSampleFile)
                        {
                            ...
                            Console.WriteLine(
                                $"Found {uiRootCandidatesAddresses.Count} candidates for UIRoot in {(int)searchUIRootsStopwatch.Elapsed.TotalSeconds} seconds: " +
                        ...
                        (IMemoryReader, IImmutableList<ulong>) GetMemoryReaderAndRootAddresses()
                        {
                            ...
                            {
                                return
                                    GetMemoryReaderAndWithSpecifiedRootFromProcessSampleFile(
                                        System.IO.File.ReadAllBytes(sourceFileArgument),
                                        ParseULong(rootAddressArgument));
                            }
                        ...
                        Console.WriteLine(
                            $"Read {uiTrees.Count} UI trees in {(int)readUiTreesStopwatch.Elapsed.TotalMilliseconds} milliseconds:" +
                            string.Join("", uiTreesReport));
    ...
    static byte[] GetProcessSampleFileFromProcessId(int processId)
    {
        ...
        return
            ProcessSample.ZipArchiveFromProcessSample(
                committedRegions,
                logEntries,
